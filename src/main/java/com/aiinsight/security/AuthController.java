...
import com.aiinsight.security.CustomOAuth2User;
...
public class AuthController {
    ...
    public ResponseEntity<?> getCurrentUser() {
        ...
        if (userId == null) {
            log.warn("Unsupported principal type in /me: {}", authentication.getPrincipal().getClass().getName());
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
    ...
    public ResponseEntity<?> refreshToken(@CookieValue(name = "refreshToken", required = false) String refreshToken,
        ...
        try {
            ...
            // Refresh 토큰도 회전시켜 만료 직전에 로그인 실패가 발생하지 않도록 함
    ...
    private Long extractUserId(Authentication authentication) {
        ...
        if (principal instanceof CustomOAuth2User customPrincipal) {
            return customPrincipal.getUser().getId();
        }
        if (authentication instanceof OAuth2AuthenticationToken oauthToken &&
                oauthToken.getPrincipal() instanceof CustomOAuth2User customOAuth2User) {
