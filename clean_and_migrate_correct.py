...
def clean_and_migrate():
    ...
    try:
        ...
        local_cur.execute("""
            SELECT id, name, url, description, selector_config, cron_expression,
                   crawl_type, enabled, last_crawled_at, last_status, created_at, updated_at
            FROM crawl_target
            ORDER BY id
        """)
        ...
        for target in local_cur.fetchall():
            try:
                railway_cur.execute("""
                    ...
                    (id, name, url, description, selector_config, cron_expression,
                     crawl_type, enabled, last_crawled_at, last_status, created_at, updated_at)
